...
public class AppStateManager : Singleton<AppStateManager>
{
    ...
    void Start()
    {
        ...
        // We start immediately by looking for an anchor
        CurrentAppState = AppState.WaitingForAnchor;
        updateStatusText();

        // Spatial mapping gets enabled once we have an anchor
        SpatialMappingManager.Instance.StopObserver();
        SpatialMappingManager.Instance.gameObject.SetActive(false);
    }
    ...
    {
        if (statusText != null) {
            TextMesh textObject = statusText.GetComponent<TextMesh>();
            textObject.text = CurrentAppState.ToString();
        }
    }
}